...
Sử dụng Lambda expression có thể làm cho code trở nên ngắn gọn, gọn gàng hơn. Giúp java cũng có thể hỗ trợ *functional programming* đơn giản.
...
java bổ sung package `java.util.stream`, nó đại khái giống với stream trước đây. Trước đây tiếp xúc nhiều nhất là resource stream, ví dụ `java.io.FileInputStream`, thông qua stream đưa file từ nơi này đến nơi khác, nó chỉ là kẻ vận chuyển nội dung, không thực hiện thao tác *CRUD* nào đối với nội dung file.
